...
void Preprocessor::preprocess(const cv::Mat& frame, cv::Mat& input_tensor) const {
    cv::Mat resized;
...
cv::Mat Postprocessor::postprocess(const std::vector<float>& output, int rows, int cols) const {
    cv::Mat depth_map = cv::Mat(rows, cols, CV_32F, (void*)output.data());
    cv::normalize(depth_map, depth_map, 0, 1, cv::NORM_MINMAX); // Normalize between 0 and 1
    return depth_map;
}

// DepthMapApp class implementation
DepthMapApp::DepthMapApp(const std::string& model_path)
    : model(model_path) {}
...
void DepthMapApp::run() {
    ...
    while (true) {
        ...
        if (cv::waitKey(1) == 27) break;  // Stop if 'ESC' is pressed
    }
